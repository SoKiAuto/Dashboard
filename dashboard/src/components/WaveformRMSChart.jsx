...
export default function WaveformRMSChart({ channel, liveData, setpoints }) {
  const [history, setHistory] = useState([]);
  const [range, setRange] = useState("1h");

  const fetchHistory = async () => {
    try {
      const res = await fetch(`/api/vm/history?channel=${channel}&metrics=Waveform_RMS&range=${range}`);
      const data = await res.json();
      setHistory(data.reverse());
    } catch (err) {
      console.error("Waveform RMS Chart error:", err);
    }
  };
  ...
  const value = liveData?.values?.Waveform_RMS ?? 0;
  const thresholds = setpoints?.Waveform_RMS ?? {};
  const color = value >= thresholds.alarm ? "bg-red-600"
              : value >= thresholds.warning ? "bg-yellow-500"
              : "bg-green-600";

  const chartData = history.map(item => ({
    value: item?.values?.Waveform_RMS ?? 0,
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold">Waveform RMS</h2>
